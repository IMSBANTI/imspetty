...
export default function CashAdvanceManager({ cashAdvances, vouchers, onSaveAdvance, onDeleteAdvance, isAdmin }) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="glass-panel rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg space-y-4 p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-base sm:text-lg font-bold font-heading text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <FileText className="w-5 h-5 text-red-500" />
            Complete Cash Book Statement & Running Balance
          </h3>
          <span className="text-[11px] text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-800 font-semibold">
            {ledgerWithBalance.length} Entries
          </span>
        </div>

        <div className="table-container border border-slate-200 dark:border-slate-800 rounded-2xl overflow-x-auto">
          <table className="custom-table text-[11px] sm:text-xs w-full">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-semibold text-[10px] sm:text-[11px] uppercase tracking-wider">
                <th className="px-2.5 py-2.5 whitespace-nowrap">Date</th>
                <th className="px-2.5 py-2.5 whitespace-nowrap">Ref / ID</th>
                <th className="px-2 py-2.5 whitespace-nowrap text-center">Type</th>
                <th className="px-2.5 py-2.5">Description</th>
                <th className="px-2.5 py-2.5 whitespace-nowrap">Source / Staff</th>
                <th className="px-2.5 py-2.5 text-right whitespace-nowrap">Cash In (+)</th>
                <th className="px-2.5 py-2.5 text-right whitespace-nowrap">Cash Out (-)</th>
                <th className="px-3 py-2.5 text-right whitespace-nowrap bg-slate-200/90 dark:bg-slate-800/90 font-bold text-slate-900 dark:text-slate-100">Running Balance (TK)</th>
                {isAdmin && <th className="px-2 py-2.5 text-center whitespace-nowrap">Action</th>}
              </tr>
            </thead>
              ...
              {ledgerWithBalance.length === 0 ? (
                ...
                ledgerWithBalance.map((tx) => (
                  <tr key={tx.id} className={tx.type === 'INCOME' ? 'bg-emerald-500/5 dark:bg-emerald-950/20' : 'hover:bg-slate-500/5'}>
                    <td className="px-2.5 py-2 font-mono text-slate-600 dark:text-slate-400 whitespace-nowrap">{tx.date}</td>
                    <td className="px-2.5 py-2 font-mono font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap">{tx.id}</td>
                    <td className="px-2 py-2 text-center whitespace-nowrap">
                      {tx.type === 'INCOME' ? (
                        <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 whitespace-nowrap">
                          <ArrowDownLeft className="w-3 h-3" /> ADV IN
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/30 whitespace-nowrap">
                          <ArrowUpRight className="w-3 h-3" /> EXP OUT
                        </span>
                      )}
                    </td>
                    <td className="px-2.5 py-2 text-slate-600 dark:text-slate-400 max-w-[150px] truncate" title={tx.description}>{tx.description}</td>
                    <td className="px-2.5 py-2 text-slate-700 dark:text-slate-300 whitespace-nowrap">{tx.person}</td>
                    <td className="px-2.5 py-2 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                      {tx.inAmount > 0 ? `+৳${tx.inAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}` : '-'}
                    </td>
                    <td className="px-2.5 py-2 text-right font-mono font-bold text-red-600 dark:text-red-400 whitespace-nowrap">
                      {tx.outAmount > 0 ? `-৳${tx.outAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}` : '-'}
                    </td>
                    <td className="px-3 py-2 text-right font-mono font-extrabold text-slate-900 dark:text-slate-100 bg-slate-100 dark:bg-slate-900 whitespace-nowrap">
