...
import { Wallet, Calendar, FileText, TrendingUp, PlusCircle, ArrowUpRight, CheckCircle2, PieChart, Landmark, ArrowDownLeft } from 'lucide-react';
...
export default function Dashboard({ vouchers, cashAdvances, onNavigateAdd, onNavigateCashIn }) {
  ...
  const currentMonthVouchers = vouchers.filter(v => v.date && v.date.startsWith(currentMonthStr));
  const currentMonthTotal = currentMonthVouchers.reduce((acc, v) => acc + (parseFloat(v.amount) || 0), 0);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="glass-panel p-8 rounded-3xl border border-red-950/40 relative overflow-hidden flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-2xl bg-gradient-to-r from-[#14161e] via-[#1a1c27] to-[#0f1118]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className={`glass-panel p-6 rounded-2xl border flex items-center justify-between relative overflow-hidden group transition-all ${
          remainingBalance < 0 ? 'border-rose-500/50 bg-rose-500/10' : 'border-red-950/40 hover:border-red-500/40'
        }`}>
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Cash Balance in Hand</p>
            <h3 className={`text-2xl font-extrabold font-mono ${remainingBalance < 0 ? 'text-rose-400' : 'text-slate-100'}`}>
              ৳{remainingBalance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
            </h3>
            <p className="text-xs text-slate-400 flex items-center gap-1 font-medium">
              <Wallet className="w-3.5 h-3.5 text-red-400" /> Fund in Petty Cash Box
            </p>
          </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-emerald-950/40 flex items-center justify-between relative overflow-hidden group hover:border-emerald-500/40 transition-all">
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Cash From Accounts</p>
        ...
        {/* Card 3: Total Spent */}
        <div className="glass-panel p-6 rounded-2xl border border-red-950/40 flex items-center justify-between relative overflow-hidden group hover:border-red-500/40 transition-all">
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Spent (Vouchers)</p>
            <h3 className="text-2xl font-extrabold font-mono text-red-400">
              ৳{totalExpenditure.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
            </h3>
            <p className="text-xs text-slate-400 font-medium">{vouchers.length} Vouchers paid out</p>
          </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-red-950/40 flex items-center justify-between relative overflow-hidden group hover:border-red-500/40 transition-all">
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Top Spending Area</p>
            <h3 className="text-lg font-bold text-slate-100 truncate max-w-[130px]" title={topCategory}>{topCategory}</h3>
      ...
      {/* Main Grid: Topsheet & Category Visual Breakdown */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Topsheet Project Table (2 Cols) */}
        <div className="lg:col-span-2 glass-panel p-6 rounded-3xl border border-red-950/40 space-y-4">
            ...
            <table className="custom-table">
              <thead>
                <tr>
                  <th>SL</th>
                  <th>Project Name</th>
                  <th className="text-right">Total Expenditure (TK)</th>
                ...
                {Object.keys(projectTotals).length === 0 ? (
                  <tr>
                    <td colSpan="4" className="text-center py-8 text-slate-500">
                      No vouchers recorded yet. Click 'Enter Voucher' to start adding data.
                    </td>
                  </tr>
                ) : (
                  Object.entries(projectTotals).map(([proj, amt], index) => {
                    const share = totalExpenditure > 0 ? ((amt / totalExpenditure) * 100).toFixed(1) : '0';
                    return (
                      <tr key={proj} className="hover:bg-red-500/10">
                        <td className="font-mono text-slate-500 font-semibold">{index + 1}</td>
                        <td className="font-bold text-slate-200">{proj}</td>
                        <td className="text-right font-bold text-red-400 font-mono">
                          ৳{amt.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="text-right font-mono text-slate-400 font-semibold">{share}%</td>
                      </tr>
                    );
                  })
                )}
        ...
        {/* Category Breakdown Cards (1 Col) */}
        <div className="glass-panel p-6 rounded-3xl border border-red-950/40 space-y-4">
          ...
          <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
            {Object.keys(categoryTotals).length === 0 ? (
              <p className="text-center text-xs text-slate-500 py-6">No category data recorded yet.</p>
            ) : (
              Object.entries(categoryTotals)
                ...
                .map(([cat, amt]) => {
                  ...
                  return (
                    <div key={cat} className="p-3 bg-[#0d0e14] rounded-xl border border-slate-800 space-y-1.5">
                      ...
                      <div className="text-[10px] text-slate-400 text-right font-mono">{pct}% of total</div>
                    </div>
                  );
                })
            )}
